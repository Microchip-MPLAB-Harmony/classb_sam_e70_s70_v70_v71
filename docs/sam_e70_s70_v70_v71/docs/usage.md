...
It uses RTT and SysTick to measure the CPU clock frequency. The RTT is clocked at 32768 Hz from the
...
**Note**
1. This self-test uses the RTT peripheral. Thus, if it is used during run-time, the RTT shall not
be used by the application for continuous modes such as real time clock or calendar. If the RTT is used
for some other purpose, it must be reconfigured after running the clock self-test.
2. This self-test configures XOSC32K to clock the RTT. If the application cannot afford to reconfigure
...
Nested Vectored Interrupt Controller (NVIC), the Real-Time Counter (RTT) and the Timer Counter 0 (TC0)
...
It reports a PASS if the RTT has generated at least one interrupt and the total number of interrupts generated
by the TC0 is greater than one and less than the specified upper limit. The clock used for RTT is 32.768kHz from the
internal SLCK and for TC0 the clock is the default CPU clock (12MHz from the MAINCK) / 2 = 6MHz.
The interrupt self-test can be used only at startup.

**Note**
1. This startup self-test utilizes the interrupts generated by RTT and TC0. For run-time testing of interrupts,
